...
define(['vueLoader'], function (loader) {
    ...
    return {
        ...
        mounted: function mounted() {
            ...
            if (!self.loaded) {
                var _selfEl = $(self.$el);
                var winHeight = app.getScrollContainer().offset().top;
                var selfOffsetY = _selfEl.offset().top;
                var selfHeight = _selfEl.height();

                self.model.cpts.forEach(function (e) {
                    self.loaded = true;
                    //如果在显示区域外，延时加载
                    if (_this2.delayLoad && e._offsetY) {
                        var _offsetY = selfOffsetY + e._offsetY - winHeight;
            ...
            $(window).on("touchmove", function () {
                ...
                var y = 50 + parseInt(scrollTop * -(0.1 + multiple)) / 10 + '%';
        ...
        methods: {
                    ...
                    var create = function create(model) {
                        ...
                        if (!v.copy) {
                            ...
                            v.copy = function (c, l, t, oldModel) {
                                self.append(c, l, t, oldModel, callback);
                            };
                        }
                        ...
                        // self.$children.push[v];
                        loader.append(self.$refs.container, v, callback);
                        if (self.afterDrop) {
                            self.afterDrop(model);
                        }

                        //手机端延后
                        if (!callback) var ca11back = function ca11back() {
                            var winHeight = loader.win().offset().top;
            ...
            check: function check(model) {
                ...
                var a = app;
                // 标题区初始化设置
                this.initProps(model, 'titleBarStyle', this.copyDefaultModel(a.titleConfig.titleBarStyle));
                this.initProps(model, 'titlePadding', this.copyDefaultModel(a.titleConfig.titlePadding));
                this.initProps(model, 'titleStyle', this.copyDefaultModel(a.titleConfig.titleStyle));
                this.initProps(model, 'subTitleStyle', this.copyDefaultModel(a.titleConfig.subTitleStyle));
                this.initProps(model, 'showMore', this.copyDefaultModel(a.titleConfig.showMore));
