...
var setVm = new Vue({
	...
	methods: {
		//查询并渲染关键字设置
		checkRender: function checkRender() {
			if (userWebId) {
		...
		//点击nav 关键字设置 
		navKeywordClick: function navKeywordClick() {
			this.flag = 'seo';
			this.checkRender();
		},
		//提交seo 关键字设置
		saveSeo: function saveSeo() {
			//判断
			if (setVm.title === '') {
		...
		// 删除网站图标
		shiftFavicon: function shiftFavicon() {
			var _this = this;
			var url = domain + saveTitleUrl,
			    data = {
				userWebId: _this.userWebId,
