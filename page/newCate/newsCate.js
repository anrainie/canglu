...
var newsVm = new Vue({
	...
	data: (_data = {
		...
		checkallFlag: false }, _defineProperty(_data, 'dataName', ''), _defineProperty(_data, 'pageSize', 10), _defineProperty(_data, 'pageIndex', ''), _defineProperty(_data, 'totalArr', []), _defineProperty(_data, 'showTotalArr', []), _defineProperty(_data, 'total', ''), _defineProperty(_data, 'totalPage', 1), _defineProperty(_data, 'jumpPage', ''), _defineProperty(_data, 'newsId', 0), _defineProperty(_data, 'recycleList', []), _defineProperty(_data, 'recycleArr', []), _defineProperty(_data, 'recycleIdArr', []), _defineProperty(_data, 'checkallFlagR', false), _defineProperty(_data, 'info', {}), _defineProperty(_data, 'popUpFlag', false), _defineProperty(_data, 'tipsFlag', false), _data),
	...
	methods: {
		//没有数据，添加新闻
		toAddNews: function toAddNews() {
			//跳去保存新闻页面
			this.showSave();
		},
		//搜索  newsBlock == 'list'
		search: function search(newsBlock) {
			if (newsBlock === 'list') {
				newsVm.getList(1, 1);
			} else {
				newsVm.getList(2, 1);
		...
		//nav文章切换 
		showNewsList: function showNewsList() {
			this.newsBlock = "list";
			this.getList(1, 1);
		},
		//显示保存新闻页面
		...
		clearSaveForm: function clearSaveForm() {
			this.topLevel = '';
			this.sonLevel = '';
			this.newsName = '';
			//隐藏阅览图片
		...
		showOneClass: function showOneClass() {
			var _this = this;
			if (!_this.levelFlag) {
		...
		showTwoClass: function showTwoClass() {
			var _this = this;
		...
		reqData: function reqData(level) {
			var _this = this;
			var url = domain + 'service/newsCate/list.html',
			    data = {
				level: level
		...
		//显示回收站
		showRecycle: function showRecycle() {
			newsVm.newsBlock = 'recycle';
			newsVm.searchFlag = true;
		},
		//一级删除一条数据
		shift: function shift(id, level) {
			if (id === 0) {
				if (level === 1) {
					//删除一级最后一条
					this.firstList.pop();
		...
		save: function save(index, level) {
			var item = {};
			if (level == 1) {
		...
		//一级二级添加分类
		addClass: function addClass(level) {
			//对比data的数组跟后台的数组是不是一样
			var url = domain + 'service/newsCate/list.html',
			    data = {
				level: level
		...
		//二级选择 聚焦效果：显示下拉框
		shouLi: function shouLi(event) {
			var el = event.currentTarget;
			var option = $(el).next().show();
		},
		//离焦效果：隐藏下拉框
		hideLi: function hideLi(event) {
			var el = event.currentTarget;
			var option = $(el).next().hide();
		},
		//二级select 选择
		choose: function choose(parentId, index, parentName, event) {
			//改变父级id
		...
		//保存页面 联动
		//聚焦
		getFather: function getFather(event) {
		...
		//离焦 隐藏下拉框
		hideSonli: function hideSonli(event) {
			var el = event.currentTarget;
			$(el).next().hide();
		...
		/*保存新闻页面部分*/
		//赋值
		gettopLevel: function gettopLevel(name, topLevelId, event, item) {
			var _this = this;
			_this.topLevel = name;
			var el = event.currentTarget;
			$(el).parent().hide();
		...
		findSecondLink: function findSecondLink() {
			var _this = this;
			//先重置二级联动数组为[]
			_this.sonLevelArr = [];
			//找出二级联动
		...
		//子集 聚焦
		findSon: function findSon(event) {
			var el = event.currentTarget;
			$(el).next().show();
			$(el).parent().addClass("is-focus");
		...
		//把子级赋值
		getsonLevel: function getsonLevel(name, sonLevelId, event) {
			this.sonLevel = name;
			var el = event.currentTarget;
			$(el).parent().hide();
		...
		//上传图片
		newsImgUpload: function newsImgUpload(previewId, event) {
		...
		//渲染文章列表和回收站列表
		getList: function getList(state, page) {
			if (page === "") {
		...
		//单纯请求列表数据 原始函数
		getDataList: function getDataList(obj) {
			var _this = this;
		...
		//总条数转为数组
		changeArr: function changeArr(n) {
			var arrTotal = [];
			for (var i = 1; i < n + 1; i++) {
				arrTotal.push(i);
			}
		...
		showPages: function showPages(n) {
			var page = parseInt(n);
		...
		//拼接新闻列表临时数组 也可以在回收站用
		getIdArr: function getIdArr(arr) {
			var newArr = [];
		...
		//单个选
		singleCheck: function singleCheck() {
			//当页列表的id临时数组长度
		...
		getFDate: function getFDate(value) {
			if (value != null && value != "") {
   ...
   * */
		deleteListArr: function deleteListArr(state) {
		...
		deleteOne: function deleteOne(id, state) {
			var _this = this;
			showToast({
				title: "提示",
				msg: "确定删除这些文章？",
		...
		//单个选
		singleCheckR: function singleCheckR() {
			//回收站当前列表的id临时数组长度
		...
		deleteRecy: function deleteRecy() {
		...
		// 参数：checkIds	产品ID集合(1,2,3)
		//state	状态（1、恢复 2、回收站）
		recover: function recover(state) {
			if (!newsVm.recycleArr.length) {
				showTips('请选择');
				return;
			}
