...
var productVm = new Vue({
    ...
    methods: {
        ...
        redelProduct: function redelProduct(id) {
        ...
        getProducts: function getProducts(type, page) {
        ...
        returnIds: function returnIds(arr) {
            var newArr = [];
        ...
        returnImgs: function returnImgs(arr) {
            var newArr = [];
            for (var i = 0; i < arr.length; i++) {
        ...
        //上一页
        prefPage: function prefPage(type, page) {
            if (page > 1) {
                page--;
            } else {
                showTips("已经是第一页了");
        ...
        //下一页
        nextPage: function nextPage(type, page) {
        ...
        //转移分类
        changeCates: function changeCates() {
            var products = productVm.checkedProduct;
        },
        //分类弹窗隐藏
        tispsTagHide: function tispsTagHide() {
            productVm.tispsTag = false;
        },
        //确认选择分类
        tispsTagComfire: function tispsTagComfire() {
            productVm.tispsTag = false;
        ...
        reShowProduct: function reShowProduct() {
            //重新渲染页面
            productVm.getProducts(1);
            productVm.checkedProduct = [];
            productVm.checkAll = false;
        },
        checkedProductFn: function checkedProductFn() {
            //当页列表下的全部商品
            var arr = productVm.productIds;
            //    获取当前已经选择的商品
        ...
        //        删除 && 恢复
        delProduct: function delProduct(id, type) {
            //根据id去判断否是批量，根据type去判断删除？恢复？
            var m = '个';
            var productsId = productVm.checkedProduct;
            var data = {
        ...
        //分类保存
        cateSave: function cateSave(index, type) {
            var _this = this;
        ...
        cateChange: function cateChange(n, cateLv) {
            //修改分类名称时记录当前存在未保存项
        ...
        cateFocus: function cateFocus(n, cateLv) {
            //点击分类名称时记录当前编辑项索引
            var cateName;
            var flag = productVm.cateSaveFlag;
        ...
        //    添加分类
        cateAdd: function cateAdd(index) {
            //添加分类，创建一个基础对象
            var newObj = {
                id: null,
                parentId: 0,
         ...
         * */
        //  获取一级分类
        getFirstCate: function getFirstCate(state) {
        ...
        //    选取一级分类
        getCatef: function getCatef(cate, id, path) {
        ...
        //    获取二级分类
        getSecondCate: function getSecondCate(state) {
            var newState = state ? false : true;
            productVm.secondCateflag = newState;
            productVm.getCate(3);
        },
        getSecondc: function getSecondc(cate, id, path) {
        ...
        //    获取三级分类
        getThirdCate: function getThirdCate(state) {
            var newState = state ? false : true;
            productVm.thirdCateflag = newState;
        },
        getThirdc: function getThirdc(cate, id, path) {
            productVm.productCateId = id;
        ...
        //选择上级分类，获取对应下级分类
        returnCates: function returnCates(arr, parentId) {
            var newArr = [];
            for (var i = 0; i < arr.length; i++) {
        ...
        closeImg: function closeImg(n) {
            productVm.checkImgs = -1;
        },
        // 上传图片
        comfireImg: function comfireImg() {
            var _this = this;
        ...
        defaultPag: function defaultPag() {
            productVm.imgI = -1;
        },
        //添加产品列表清空
