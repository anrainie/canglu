...
var messageVm = new Vue({
    ...
    methods: {
        ...
        messageList: function messageList(type, page) {
            //加载留言板列表
            var url = domain + 'service/message/list.html';
            var data = {
                state: type,
                userWebId: userWebId,
                page: page,
        ...
        viewDetails: function viewDetails(detailsId) {
            //查看详情
            $("#messageDetails").show();
            var id = detailsId;
            var url = domain + 'service/message/' + id + '.html';
            var data = {
                id: id,
                ssid: ssid
            };
        ...
        respondComments: function respondComments() {
            //回复留言板
            var id = messageVm.messageDetails.id;
            var url = domain + 'service/message/reply.html';
            var data = {
                msgId: id,
                ssid: ssid,
                // userWebId:userWebId,
        ...
        //        删除 && 恢复
        delMessage: function delMessage(id, type) {
        ...
        redelMessage: function redelMessage(id) {
            //根据id去判断否是批量，根据type去判断删除？恢复？
        ...
        reShowMessage: function reShowMessage() {
            //重新渲染页面
            messageVm.messageList(1);
            messageVm.checkedNames = [];
            messageVm.checkAll = false;
        },
        checkedMessageFn: function checkedMessageFn() {
            //当页列表下的全部商品
