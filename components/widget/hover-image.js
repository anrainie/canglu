        ...
        computed: {
            imageSrc: function imageSrc() {
                ...
                if (_typeof(this.src) == 'object') {
                    if (this.hover && this.src.hsrc && this.hoverEffect == 7) v = this.src.hsrc;else {
                        v = this.src.src;
                    }
                } else {
                    v = this.src;
        ...
        watch: {
            'hover': {
                handler: function handler() {
                    var self = this;
                    if (self.hover == true) {
                        if (self.hoverEffect == 1) {
                            self.$refs.hoverImg.style.width = 'calc(100% - 2px)';
                            self.$refs.hoverImg.style.height = 'calc(100% - 2px)';
                        } else if (self.hoverEffect == 5) {
                            var h = void 0;
                            if (self.model.imageContainerStyle) h = self.model.imageContainerStyle.height;
                            if (self.$el.style.height) h = self.$el.style.height;else {
                                h = self.src.style.height;
                            }
                            self.LH = h;
                        } else if (self.hoverEffect === 6) {
                            var _h = void 0;
                            if (self.model.imageContainerStyle) _h = self.model.imageContainerStyle.height;
                            if (self.$el.style.height) _h = self.$el.style.height;else {
                                _h = self.src.style.height;
                            }
                            self.LH = parseInt(_h) * 0.3 + 'px';
                        }
                    }
                    if (self.hover == false) {
                        if (self.hoverEffect == 1) {
                            self.$refs.hoverImg.style.width = '100%';
                            self.$refs.hoverImg.style.height = '100%';
                        }
                    }
                },
                deep: true
