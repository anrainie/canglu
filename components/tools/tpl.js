...
define(['util'], function (util) {
    return {
        ...
        methods: {
            select: function select(webType) {
                if (this.tempaltesData.themeCode == webType.code) this.tempaltesData.themeCode = '';else {
                    this.tempaltesData.themeCode = webType.code;
                }
                this.showTplIndustry();
            },
            ...
            /** 选择模板 **/
            checkModle: function checkModle(id, themeCode) {
                var _this = this;
                this.modelId = id;
            ...
            // 确认选择默认
            comfirms: function comfirms() {
                var self = this;
                if (self.determine) return;
                self.determine = true;
                var url = 'service/system/web/chose.html';
                var data = {
            ...
            // 匹配一级分类是否处在选中状态
            firstWebType: function firstWebType(code, parentCode) {
                var _this = this;
            ...
            //选择行业分类
            chosenOneLevel: function chosenOneLevel(index, code, parentCode) {
                var _this = this;
                if (parentCode == 0) {
                    // 一级分类
            ...
            //选择模板主题颜色
            chosenTplThemeColor: function chosenTplThemeColor(tplThemeColorCode) {
                var _this = this;
            ...
            checkOrderBy: function checkOrderBy(str) {
                var _this = this;
            ...
            //模板网站预览
            previewWebTpl: function previewWebTpl(sysWebId) {
            ...
            hover: function hover(index) {
                var box = document.getElementsByClassName('operating')[index];
                var img = document.getElementsByClassName('img')[index];
                box.classList.add('ani');
                img.classList.add('imgAni');
            },
            out: function out() {
                var box = document.getElementsByClassName('ani')[0];
                var img = document.getElementsByClassName('imgAni')[0];
                box.classList.remove('ani');
                img.classList.remove('imgAni');
            },
            ahover: function ahover(index) {
                var ele = this.$refs['program'][index];
                ele.classList.add('prohover');
            },
            aout: function aout(index) {
                var ele = this.$refs['program'][index];
                ele.classList.remove('prohover');
            }
        }
    };
});