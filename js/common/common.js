...
var CL = {
	...
	createModuleid: function createModuleid() {
		var id = Math.random();
	},
	addScript: function addScript(option) {
  ...
  */
	...
	initScript: function initScript(option) {
		...
		if (option.src) {
			...
			var tabulationUrl = 'service/' + option.moduleInit + '/list.html';
			var tabulationData = {};
		...
		//给模块外层加外壳
		var _module = '<div data-type="publicModule" id="';
	...
	objSlideDown: function objSlideDown(obj, goTop) {
		obj.css({
			'position': 'absolute',
			'top': '-99999px'
		});
		...
		if (goTop) {
			...
			var objPrev = $('#editContent').children().eq(obj.index() - 1);
	...
	recoverStyle: function recoverStyle(value) {
		if (value == null) return null;
	...
	// 创建一个组件盒子
	setModuleContainer: function setModuleContainer(option) {
		//如果需要初始化就拼接初始化的代码
		var _script = '';
	...
	close: function close(dialog, callBack) {
	...
	openDialog: function openDialog(obj, beforeFn) {
